...
import type { Cell, Rows, SonoTableNode } from "./TableNode";
import {
  $isTableNode,
  cellHTMLCache,
...
import { IS_APPLE } from "@/components/GeneralComponents/LexicalEditor/shared/src/environment";
import { CellContext } from "./SonoTablePlugin";
...
function $updateCells(
  ...
  updateTableNode: (fn2: (tableNode: SonoTableNode) => void) => void,
  fn: () => void
): void {
  for (const id of ids) {
...
export default function TableComponent({
  ...
  const updateTableNode = useCallback(
    (fn: (tableNode: SonoTableNode) => void) => {
      editor.update(() => {
    ...
    const copyCellRange = (event: ClipboardEvent) => {
      ...
      if (
        ...
        const tableNodeJSON = editor.getEditorState().read(() => {
          const tableNode = $getNodeByKey(nodeKey) as SonoTableNode;
