...
export class SonoTableNode extends DecoratorNode<JSX.Element> {
  __rows: Rows;

  static getType(): string {
    return "sonotablesheet";
  }

  static clone(node: SonoTableNode): SonoTableNode {
    return new SonoTableNode(Array.from(node.__rows), node.__key);
  }

  static importJSON(serializedNode: SerializedTableNode): SonoTableNode {
...
): node is SonoTableNode {
  return node instanceof SonoTableNode;
}

export function $createTableNode(rows: Rows): SonoTableNode {
  return new SonoTableNode(rows);
}
